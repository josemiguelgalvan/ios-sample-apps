...
(function () {
  ...
  function makeStandardEvent(event) {
    return {eventName : event.name, time : (new Date()).toISOString()};
  }
  ...
  postHttpRequest(this._iqEndpoint, this._base);
  this._pendingEvents = [];
  ...
  this._base = params;
  this._base.asset = {};
  this._base.sessionId = generateUUID();
  this._base.pcode = pCode;
  this._base.sessionStartTime = (new Date()).toISOString();
  this._base.analyticsSdkName = "ooyala-tvos-one-analytics-sdk";
  ...
  this._flushTime = Number.MAX_VALUE;
  this._timePlayed = 0;
  this._iqEndpoint = "http://l.ooyala.com/v3/analytics/events";
...
function createReporter(pcode, params) {
  console.log("create reporter at" + Date.now());
  return new Ooyala.Reporter(pcode, params);
}
